...
const sampleEntries: ContentHistoryEntry[] = [
  {
    id: "h1",
    sourceId: "n1",
...
function renderModal(overrides: Partial<Parameters<typeof NoteHistoryModal>[0]> = {}) {
  const props = {
    open: true,
    onClose: vi.fn(),
...
describe("NoteHistoryModal", () => {
  it("calls onRevert with entry content after confirming revert", () => {
    const props = renderModal();
    const revertButtons = screen.getAllByRole("button", { name: /revert/i });
    fireEvent.click(revertButtons[0]);
    // Confirmation dialog appears — click the confirm Revert button
    const confirmButton = screen.getByRole("button", { name: "Revert" });
